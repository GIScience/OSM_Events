...
boxplot(events, 'GeomID', 'contributions', pdir+'/outputs') # produce boxplot figure for number of events per cell (Figure 3)
...
events = months_df[months_df.event==1]
events.to_csv(pdir+'/outputs/OSM_large_scale_events.csv')
...
create_events_table(events, 'label', ['Creation_ratio', 'Deletion_ratio', 'Tag_ratio', 'Geom_ratio', 
                                        'Max_ratio', 'users', 'timestep'], ['timestep', 'users'], pdir+'/outputs')
...
create_weights_table(events, totals, 'label', ['contributions', 'creations', 'deletions', 'tagChanges', 'GeometryChanges'], pdir+'/outputs')
del totals
...
draw_time_figure(month_totals, event_totals, labels, pdir+'/outputs')
...
event_weights_by_cell.to_csv(pdir+'/outputs/event_weights_by_cell.csv')
...
compute_effects(months_df, 'GeomID', 'timestep', 'event', 'label',
                ...
                labels, [6, 12], pdir+'/outputs', ['Operations', 'Creations', 'Deletions', 'Tag Changes', 'Geom. Changes', 'Users'],
                ['k', 'grey'])

# event effects on subsequent events
print('Computing initial event effects on the probability for subsequent events...')
compute_event_prob(events, 'GeomID', 'label', 'timestep', labels, pdir+'/outputs')
